...
const CACHE_NAME = 'wazakkir-v2.19.0';
const STATIC_ASSETS = [
  'icons/icon-192.png',
  'icons/icon-512.png',
  'icons/favicon.png',
...
self.addEventListener('fetch', event => {
  ...
  // لا تتدخل الـ service worker في طلبات Firebase (تسجيل الدخول / لوحة الأدمن)
  // خليها تروح للشبكة مباشرة زي ما هي عشان تفضل شغالة لايف دايمًا
  if (
  ...
  // Cache-first for static assets (icons, fonts, تلاوات القرآن...). ملحوظة:
  // طلبات الصوت من مواقع خارجية (everyayah.com, mp3quran.net) بتيجي كـ
  // "opaque response" (status=0) لأنها cross-origin بدون CORS، فمينفعش
  // نتأكد من status===200 زي باقي الملفات — لازم نكاشها برضه عشان الاستماع
  // بدون نت يشتغل فعلًا، فبنتأكد بس إن الطلب نجح (مفيش استثناء) قبل الكاش
  event.respondWith(
